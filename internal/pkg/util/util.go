...
func ConfigureReloaderFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().StringVar(&options.ConfigmapExcludeReloaderAnnotation, "configmap-exclude-annotation", "configmaps.exclude.reloader.stakater.com/reload", "annotation to exclude configmaps from being watched, specified by name")
	cmd.PersistentFlags().StringVar(&options.SecretExcludeReloaderAnnotation, "secret-exclude-annotation", "secrets.exclude.reloader.stakater.com/reload", "annotation to exclude secrets from being watched, specified by name")
	cmd.PersistentFlags().StringVar(&options.SecretProviderClassExcludeReloaderAnnotation, "secretproviderclass-exclude-annotation", "secretproviderclasses.exclude.reloader.stakater.com/reload", "annotation to exclude secretproviderclasses from being watched, specified by name")
	cmd.PersistentFlags().StringVar(&options.RolloutStrategyAnnotation, "rollout-strategy-annotation", "reloader.stakater.com/rollout-strategy", "annotation to define the update strategy of an Argo Rollouts rollout")
	cmd.PersistentFlags().StringVar(&options.LastReloadedFromAnnotation, "last-reloaded-from-annotation", options.LastReloadedFromAnnotation, "annotation set by Reloader to describe the resource that triggered the last reload")
